...
bibliography: 2_coalescence_CK.bib
link-citations: yes
header-includes:
    - \usepackage{lscape}
...
<!-- ---------------- -->
<!--  -->
<!-- - Very brief summary of the coalescent: -->
<!--     - a model for the evolution of genes -->
<!--     - a genealogy of genes, not a phylogeny, not a genealogy of individuals -->
<!--     - a backward in time process -->

<!-- En fait je pense que c'est assez inutile de rentrer dans ce genre de détails -->
<!-- - Very brief summary of how the coalescent is used: -->
<!--     - we don't *know* what is the real genealogy of genes, but we can estimate it -->
<!--     - comparison of genealogies: very difficult -->
<!--     - solution: summarize the information using summary statistics -->

<!--  -->
<!-- # The Package -->
<!--  -->
<!-- - Quick overview of the coala package: -->
<!--     - the simulator available -->
<!--     - the mutation model we will be using (infinite site model) -->
...
<!-- # Task 1 -->
<!-- # Make constant population simulations -->
<!-- # Based on simulations, can you say something about your observed data? -->
<!-- #  -->
<!-- # Task 2 -->
<!-- # Experiment with the simulations -->
<!-- # Can you find a demographic model that would fit better the observed data? -->
<!-- # -->
<!-- # Task 3 -->
<!-- # What problems may the above mentioned approach have? -->

<!-- [> 2. How different processes affect the coalescent <] -->
<!-- [>  <] -->
<!-- [> - Brief presentation of some of the processes or population characteristics we are <] -->
...
model_decline <- coal_model(sample_size = 20,
                           loci_number = 1000,
                           loci_length = 100) +
      feat_growth(rate = -50, time = 100) +
      feat_mutation(5) +
      sumstat_trees() +
      sumstat_sfs() +
      sumstat_nucleotide_div() +
...
for (i in 1:6) {
    barplot(simulate(model_decline)$sfs,
            main=paste("SFS", i))
}
...
par(mfrow = c(1,1))
tajimas_d_decline <-
    simulate(model_decline)$tajimas_d

plot(density(tajimas_d_decline, na.rm = T),
     main = "Distribution of Tajiam's D in a declining population")

stat_decline <- simulate(model_decline)
pi_decline <- stat_decline$pi

plot(density(pi_decline, na.rm = T),
